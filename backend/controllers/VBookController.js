const VBook = require("../models/VBookModel");

/// Get all bookings
const getAllBookings = async (req, res) => {
  let bookings;

  try {
    bookings = await VBook.find();
  } catch (err) {
    console.log(err);
  }

  if (!bookings) {
    return res.status(500).json({ message: "Error fetching bookings" });
  }

  return res.status(200).json({ bookings });
};

/// Add new booking
const addBooking = async (req, res) => {
  const { Name, Sdate, Edate, Rtime, Features, PaymentMethod, PaymentGateway, TransactionId, Amount } = req.body;
  const License = req.file ? [req.file.path] : [];

  console.log("📥 Received booking data:", { Name, Sdate, Edate, Rtime, Features, PaymentMethod, PaymentGateway, TransactionId, Amount });

  let booking;

  try {
    booking = new VBook({ 
      Name,
      License, 
      Sdate, 
      Edate, 
      Rtime, 
      Features,
      PaymentMethod: PaymentMethod || 'Cash',
      PaymentStatus: PaymentMethod === 'Online' && TransactionId ? 'Completed' : 'Pending',
      PaymentGateway: PaymentGateway || 'None',
      TransactionId,
      Amount
    });
    await booking.save();
    console.log("✅ Booking saved successfully:", booking);
  } catch (err) {
    console.log("❌ Error saving booking:", err);
    return res.status(500).json({ message: "Error adding booking", error: err.message });
  }

  if (!booking) {
    return res.status(404).json({ message: "Unable to add booking" });
  }

  return res.status(200).json({ booking });
};

/// Get booking by ID
const getById = async (req, res) => {
  const id = req.params.id;
  try {
    const booking = await VBook.findById(id);
    if (!booking) {
      return res.status(404).json({ message: "Booking not found" });
    }
    return res.status(200).json({ booking });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ message: "Error fetching booking", error: err.message });
  }
};

/// Update booking
const updateBooking = async (req, res) => {
  const id = req.params.id;
  const { Name, Sdate, Edate, Rtime, Features, PaymentMethod, PaymentStatus, PaymentGateway, TransactionId, Amount } = req.body;
  const License = req.file ? [req.file.path] : undefined;

  let booking;

  try {
    const updateData = {
      ...(License && { License }), // update if new license image uploaded
      Name,
      Sdate,
      Edate,
      Rtime,
      Features,
    };

    // Add payment fields if provided
    if (PaymentMethod) updateData.PaymentMethod = PaymentMethod;
    if (PaymentStatus) updateData.PaymentStatus = PaymentStatus;
    if (PaymentGateway) updateData.PaymentGateway = PaymentGateway;
    if (TransactionId) updateData.TransactionId = TransactionId;
    if (Amount) updateData.Amount = Amount;

    booking = await VBook.findByIdAndUpdate(id, updateData, { new: true });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ message: "Error updating booking", error: err.message });
  }

  if (!booking) {
    return res.status(404).json({ message: "Unable to update booking" });
  }

  return res.status(200).json({ booking });
};

/// Delete booking
const deleteBooking = async (req, res) => {
  const id = req.params.id;
  let booking;

  try {
    booking = await VBook.findByIdAndDelete(id);
  } catch (err) {
    console.log(err);
  }

  if (!booking) {
    return res.status(404).json({ message: "Unable to delete booking" });
  }

  return res.status(200).json({ booking });
};

// Export functions
exports.getAllBookings = getAllBookings;
exports.addBooking = addBooking;
exports.getById = getById;
exports.updateBooking = updateBooking;
exports.deleteBooking = deleteBooking;