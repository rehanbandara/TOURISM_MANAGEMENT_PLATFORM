...
const StaffChangePassword = () => {
  ...
  const handleChangePassword = async () => {
    if (!password || !confirm) return showPopup("❌ Fill all fields", "error");
    if (password !== confirm) return showPopup("❌ Passwords do not match", "error");

    try {
      await axios.post("http://localhost:5000/api/staff/change-password", {
        email,
      ...
      showPopup(err.response?.data?.message || "❌ Failed to update password", "error");
    }
  };
